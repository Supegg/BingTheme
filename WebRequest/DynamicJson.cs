...
namespace Codeplex.Data
{
    public class DynamicJson : DynamicObject
    {
        ...
        public readonly XElement JsonXml;
        readonly JsonType jsonType;

        /// <summary>create blank JSObject</summary>
        public DynamicJson()
        {
            JsonXml = new XElement("root", CreateTypeAttr(JsonType.@object));
        ...
        {
            return IsObject && (JsonXml.Element(name) != null);
        }

        /// <summary>has property or not</summary>
        public bool IsDefined(int index)
        {
            return IsArray && (JsonXml.Elements().ElementAtOrDefault(index) != null);
        }

        /// <summary>delete property</summary>
        public bool Delete(string name)
        {
            var elem = JsonXml.Element(name);
            if (elem != null)
            {
                elem.Remove();
                return true;
        ...
        {
            var elem = JsonXml.Elements().ElementAtOrDefault(index);
            if (elem != null)
            {
                elem.Remove();
                return true;
        ...
        private object DeserializeObject(Type targetType)
        {
            ...
            foreach (var item in JsonXml.Elements())
            {
        ...
        private object DeserializeArray(Type targetType)
        {
            if (targetType.IsArray) // Foo[]
            {
                ...
                dynamic array = Array.CreateInstance(elemType, JsonXml.Elements().Count());
                var index = 0;
                foreach (var item in JsonXml.Elements())
                {
        ...
        public override bool TryConvert(ConvertBinder binder, out object result)
        {
            ...
            {
                var ie = (IsArray)
                    ? JsonXml.Elements().Select(x => ToValue(x))
                    : JsonXml.Elements().Select(x => (dynamic)new KeyValuePair<string, object>(x.Name.LocalName, ToValue(x)));
        ...
        {
            return (IsArray)
                ? TryGet(JsonXml.Elements().ElementAtOrDefault((int)indexes[0]), out result)
                : TryGet(JsonXml.Element((string)indexes[0]), out result);
        }

        public override bool TryGetMember(GetMemberBinder binder, out object result)
        {
            return (IsArray)
                ? TryGet(JsonXml.Elements().ElementAtOrDefault(int.Parse(binder.Name)), out result)
                : TryGet(JsonXml.Element(binder.Name), out result);
        }

        private bool TrySet(string name, object value)
        {
            var type = GetJsonType(value);
            var element = JsonXml.Element(name);
            if (element == null)
            {
                JsonXml.Add(new XElement(name, CreateTypeAttr(type), CreateJsonNode(value)));
            }
            else
            {
        ...
        {
            var type = GetJsonType(value);
            var e = JsonXml.Elements().ElementAtOrDefault(index);
            if (e == null)
            {
                JsonXml.Add(new XElement("item", CreateTypeAttr(type), CreateJsonNode(value)));
            }
            else
            {
        ...
        {
            return (IsArray)
                ? JsonXml.Elements().Select((x, i) => i.ToString())
                : JsonXml.Elements().Select(x => x.Name.LocalName);
        }
        ...
        public override string ToString()
        {
            ...
            foreach (var elem in JsonXml.Descendants().Where(x => x.Attribute("type").Value == "null"))
            {
                elem.RemoveNodes();
            }
            return CreateJsonString(new XStreamingElement("root", CreateTypeAttr(jsonType), JsonXml.Elements()));
        }
    }
}